...
$(document).ready(function () {
  ...
  var m = document.getElementsByClassName("playlist-card");
  for (i = 0; i < playlistData.length; i++) {
    var card = createPlaylistCard(playlistData[i]);
    $("#playlist-wrapper").append(card);
  }

  for (var i = 0; i < m.length; i++) {
    if (i == 0) {
      m[0].classList.add("active-card");
    }
    m[i].onclick = function () {
      var x = document.getElementsByClassName("active-card");
      ...
      for (var k = 0; k < videoPlaySectionData.length; k++) {
        if (k == xyz) {
          ...
          // console.log(iframe);
          // console.log(playlistData[k-1].thumbnail);
        }
      }
    };
  }
